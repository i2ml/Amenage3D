...
namespace ErgoShop.Interactable
{
    ...
    public class ElementArrowsScript : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                foreach (var g in m_allArrows) g.SetActive(false);
                return;
            }
            ...
            if (show)
            {
                ...
                if (left != currentArrow) left.transform.position = oLeft;
                if (right != currentArrow) right.transform.position = oRight;
                if (upLeft != currentArrow) upLeft.transform.position = oUpLeft;
                if (upRight != currentArrow) upRight.transform.position = oUpRight;
                if (downLeft != currentArrow) downLeft.transform.position = oDownLeft;
                if (downRight != currentArrow) downRight.transform.position = oDownRight;
            }
        ...
        private void SetSizeCompoAndDirection(Vector3 dif, Vector3 inDirection, Vector3 inSizeCompo,
            out Vector3 direction, out Vector3 sizeCompo)
        {
