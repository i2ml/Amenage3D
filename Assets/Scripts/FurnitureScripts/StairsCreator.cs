...
namespace ErgoShop.Managers
{
    ...
    public class StairsCreator : CreatorBehaviour
    {
        ...
        /// <returns></returns>
        public GameObject GenerateStairs(int steps, float width, float height, float depth, bool sidesGoToFloor,
            bool generateBack = true, bool platformsOnly = false)
        {
            var stairs = pb_ShapeGenerator.StairGenerator(steps, width, height, depth, sidesGoToFloor, generateBack,
                platformsOnly);
        ...
        /// <returns></returns>
        public GameObject GenerateCurvedStairs(float innerRadius, float circumference, int steps, float width,
            float height, bool sidesGoToFloor, bool toTheLeft)
        {
            var stairs = pb_ShapeGenerator.CurvedStairGenerator(width, height, innerRadius,
                toTheLeft ? -circumference : circumference, steps, sidesGoToFloor);
            return stairs.gameObject;
        }
        ...
        {
            while (m_stairs.Count > 0) DestroyStairs(m_stairs.First());
            m_stairs = new List<Stairs>();
            foreach (var s in floor.Stairs) m_stairs.Add(s);
            foreach (var s in m_stairs) s.RebuildSceneData();
        }
        ...
        public Stairs GetStairsFromGameObject(GameObject go)
        {
            ...
            foreach (var s in m_stairs)
                for (var i = 0; i < s.associated2DObject.transform.childCount; i++)
        ...
        public override Element CopyPaste(Element stairs)
        {
            ...
            var newStairs = s.GetCopy() as Stairs;
