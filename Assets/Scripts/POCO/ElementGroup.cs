...
namespace ErgoShop.POCO
{
    ...
    public class ElementGroup : Element
    {
        ...
        public override Element GetCopy()
        {
            ...
            {
                Element copyElement = SelectedObjectManager.Instance.CopyPasteElementWithManager(e);
        ...
        public ElementGroup GetGroupFromGameObject(GameObject go)
        {
            foreach (var a in Elements)
            {
                ...
                {
                    ElementGroup res = (a as ElementGroup).GetGroupFromGameObject(go);
                    if (res != null) return res;
                }
                if (a.associated2DObject == go || a.associated3DObject == go)
                {
                    return this;
                }
            };
        ...
        public ElementGroup GetGroupFromElement(Element e)
        {
            ...
            else
            {
                foreach(var a in Elements)
                {
                    if(a is ElementGroup)
                    {
                        ElementGroup res = (a as ElementGroup).GetGroupFromElement(e);
                        if (res != null) return res;
                    }
                }
            }
        ...
        {
            string clipboard = "=== Groupe contenant " + Elements.Count + " éléments ===\n";
            foreach (var e in Elements)
            {
                clipboard += e.GetDescription() + "\n";
            }
