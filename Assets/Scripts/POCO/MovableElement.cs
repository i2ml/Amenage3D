    ...
    public abstract class MovableElement : Element
    {
        ...
        private void AdjustCurrentFurnitureWallPos3D(Vector3 pos3D, GameObject go, Vector3 startingPos)
        {
            ...
            {   //GetComponent<Rigidbody>().MovePosition
                ...
                associated2DObject.transform.position = VectorFunctions.GetExactPositionFrom3DObject(associated2DObject, associated3DObject, associated3DObject.transform.position);
                //VectorFunctions.Switch3D2D(associated3DObject.transform.position);

                Wall w = WallsCreator.Instance.GetWallFromGameObject(go);

                Vector3 perp = w.GetPerpendicularFromPos(pos3D);

                associated3DObject.transform.rotation = Quaternion.FromToRotation(Vector3.forward, perp);
                associated3DObject.transform.localEulerAngles = new Vector3(0, associated3DObject.transform.localEulerAngles.y, EulerAngles.z);
            ...
            else
            {
                ...
                {
                    associated3DObject.transform.position = pos3D; //.GetComponent<Rigidbody>().MovePosition(associated3DObject.transform.position + (pos3D - startingPos));
                }
                associated2DObject.transform.position = this is HelperElement ?
                    VectorFunctions.Switch3D2D(associated3DObject.transform.position):
                    VectorFunctions.GetExactPositionFrom3DObject(associated2DObject, associated3DObject, associated3DObject.transform.position);
            }
        ...
        public virtual void Move(Vector3 startingPos)
        {
            ...
            {  
                case (int)ErgoLayers.Top:
                    ...
                    {                        
                        ...
                        if (associated3DObject)
                        {
                            ...
                            {
                                GameObject potentialFurniture = InputFunctions.GetHoveredObject2D(GlobalManager.Instance.cam2DTop.GetComponent<Camera>(),associated2DObject.name, true);
                            ...
                            associated2DObject.transform.position= new Vector3(associated2DObject.transform.position.x, associated2DObject.transform.position.y, -y/5f);
                ...
                case (int)ErgoLayers.ThreeD:
                    GameObject go;
                    Vector3 pos3D = InputFunctions.GetWorldPoint(out go, cam, associated3DObject, tags.ToArray());
