    ...
    public class OperationsBufferScript : MonoBehaviour
    {
        ...
        public string GetLastAutoSaveContent()
        {
            if (m_projects.Count > 0)
                return File.ReadAllText(Path.Combine(ProjectManager.Instance.GetAutoSaveFolderPath(),
                    m_projects.Last().ProjectPath));
            return "";
        }
        ...
        public void AddAutoSave(string operationName)
        {
            ...
            var auto = ProjectManager.Instance.SaveCurrentFileAsAutoSave();
            if (auto != null)
            {
                ...
                var autoJsonString = "";
                if (m_projects.Count > 0)
                    lastJsonString = File.ReadAllText(Path.Combine(ProjectManager.Instance.GetAutoSaveFolderPath(),
                        m_projects.Last().ProjectPath));
                if (!string.IsNullOrEmpty(lastJsonString))
                    autoJsonString = File.ReadAllText(Path.Combine(ProjectManager.Instance.GetAutoSaveFolderPath(),
                        auto.ProjectPath));
                ...
                if (autoJsonString != lastJsonString || lastJsonString == "")
                {
                    ...
                    if (m_projects.Count > CANCEL_LIMIT_COUNT) m_projects.Remove(m_projects[0]);
                }
            }
        ...
        {
            var newList = new List<AutoSave>();
            for (var i = 0; i <= Mathf.Min(m_projects.Count - 1, m_idx); i++) newList.Add(m_projects[i]);
            m_projects.Clear();
