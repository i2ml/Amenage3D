...
namespace ErgoShop.Managers
{
    ...
    public class HelpersCreator : CreatorBehaviour
    {
        ...
        {
            while (m_helpers.Count > 0)
            {
                DestroyHelpers(m_helpers.First());
            }
            m_helpers = new List<HelperElement>();
            foreach (var tze in floor.TextZoneElements)
            {
                m_helpers.Add(tze);
            }
        ...
        {
            TextZoneElement newTZ = tz.GetCopy() as TextZoneElement;
            m_helpers.Add(newTZ);
        ...
        private void CreateTextZone()
        {
            if (m_isCreating)
            {
                ...
                else
                {
                    ...
                    (currentElement as TextZoneElement).Size = VectorFunctions.Switch2D3D(currentElement.associated2DObject.GetComponent<TextZoneScript>().SetSize(m_startPoint, m_endPoint));
